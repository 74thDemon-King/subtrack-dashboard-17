import { createFileRoute, useNavigate } from "@tanstack/react-router";
import { useEffect, useMemo, useState } from "react";
import { AlertTriangle, Info, CheckCircle2, Sparkles } from "lucide-react";
import { Card } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { toast } from "sonner";
import { useSubscriptions } from "@/hooks/useSubscriptions";
import { buildInsights } from "@/lib/derive";
import { EmptyState } from "@/components/subtrack/EmptyState";
import { PageSkeleton } from "@/components/subtrack/Loaders";

export const Route = createFileRoute("/_shell/insights")({
  head: () => ({ meta: [{ title: "Insights — SubTrack" }, { name: "description", content: "Personalised recommendations to trim recurring spend." }] }),
  component: InsightsPage,
});

const iconMap = {
  warning: { icon: AlertTriangle, bg: "bg-amber-500/10", fg: "text-amber-600", label: "Attention" },
  info: { icon: Info, bg: "bg-primary/10", fg: "text-primary", label: "Info" },
  success: { icon: CheckCircle2, bg: "bg-emerald-500/10", fg: "text-emerald-600", label: "Opportunity" },
};

const STORAGE_KEY = "subtrack.dismissed-insights.v2";

function InsightsPage() {
  const navigate = useNavigate();
  const { subs, ready, update, seedSamples } = useSubscriptions();
  const [dismissed, setDismissed] = useState<string[]>([]);

  useEffect(() => {
    try { setDismissed(JSON.parse(window.localStorage.getItem(STORAGE_KEY) || "[]")); } catch { /* ignore */ }
  }, []);

  const insights = useMemo(() => buildInsights(subs), [subs]);

  const dismiss = (id: string) => {
    const next = [...dismissed, id];
    setDismissed(next);
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(next));
    toast("Insight dismissed");
  };

  const act = async (id: string) => {
    const insight = insights.find((item) => item.id === id);
    if (!insight) return;
    if (insight.target.kind === "cancel") {
      const subId = insight.target.subId;
      const sub = subs.find((item) => item.id === subId);
      try {
        await update(subId, { status: "Cancelled" });
        toast.success(`${sub?.name ?? "Subscription"} marked as cancelled`);
      } catch {
        toast.error("Could not update this subscription");
      }
      return;
    }
    void navigate(insight.target.to === "/subscriptions" ? { to: "/subscriptions", search: { q: "" } } : { to: insight.target.to });
  };

  const visibleInsights = insights.filter((insight) => !dismissed.includes(insight.id));

  if (!ready) return <PageSkeleton cards={2} chart={false} />;

  return (
    <div className="mx-auto max-w-6xl animate-fade-up space-y-6">
      <div>
        <h1 className="flex items-center gap-2 text-2xl font-semibold tracking-tight">
          <Sparkles className="h-5 w-5 text-primary" /> Insights
        </h1>
        <p className="text-sm text-muted-foreground">Recommendations generated from your own subscription data.</p>
      </div>

      {subs.length === 0 && <EmptyState onSeed={seedSamples} />}

      <div className="grid gap-4 md:grid-cols-2">
        {visibleInsights.map((i) => {
          const cfg = iconMap[i.severity];
          const Icon = cfg.icon;
          return (
            <Card key={i.id} className="animate-fade-up rounded-2xl border-border/60 p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
              <div className="flex items-start gap-4">
                <div className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl ${cfg.bg} ${cfg.fg}`}>
                  <Icon className="h-5 w-5" />
                </div>
                <div className="min-w-0 flex-1">
                  <Badge variant="secondary" className="rounded-full text-[10px]">{cfg.label}</Badge>
                  <h3 className="mt-2 text-base font-semibold leading-snug">{i.title}</h3>
                  <p className="mt-1 text-sm text-muted-foreground">{i.detail}</p>
                  <div className="mt-4 flex gap-2">
                    <Button size="sm" className="rounded-lg" onClick={() => void act(i.id)}>{i.action}</Button>
                    <Button size="sm" variant="ghost" className="rounded-lg text-muted-foreground" onClick={() => dismiss(i.id)}>Dismiss</Button>
                  </div>
                </div>
              </div>
            </Card>
          );
        })}
        {subs.length > 0 && visibleInsights.length === 0 && (
          <Card className="p-8 text-center text-sm text-muted-foreground md:col-span-2">You're all caught up.</Card>
        )}
      </div>
    </div>
  );
}
